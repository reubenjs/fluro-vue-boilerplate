...
<template>
    <div>
        ...
        <v-container v-if="meta" fluid class="page-header">
            <h1 class="text-capitalize">{{item.title}} <span class="text-muted">// Tag</span></h1>
...
export default {
    ...
    computed:{
        seoTitle() {
            return this.item ? this.item.title : 'Keyword';
        }
    },
    methods:{
        routeChanged() {
            ...
            var promise = this.$fluro.api.get(`/photos/keyword/${self.$route.params.id}`, {
                params:{
                    // populate:[
                    //     'data.event',
                    //     'data.photographer',
