<template>
    <div>
        <v-container fluid class="page-header">
            <h1>Explore <span class="text-muted">// Tags</span></h1>
            <div v-if="loaded">Found {{items.length}} tags</div>
            <div v-if="!loaded">
                    ...
                    <template slot-scope="props">
                        <v-layout row wrap>
                            <v-flex d-flex xs12 sm4 md3 xl2 v-for="item in props.page">
                                <tag-teaser :item="item"></tag-teaser>
                            </v-flex>
...
export default {
    ...
    methods: {
        fetchData() {
            ...
            var promise = this.$fluro.api.get(`/photos/tag/list`, {
                params: {
